...
#define LOG_MAX_SHARED 13      //  hyperparameter : we can handle in shared memory (2^13 = 8192 elements) - 2 ^ 14 might be outside H100 GPU's limit 48KB
...
__global__ void bitonic_sort_shared_initial(DTYPE *data, int elements_per_thread, int total_size) {
    ...
    for (int i = 0; i < elements_per_thread; i++) {
        ...
        if (base_idx >= total_size) {
            ...
            shared_mem[threadIdx.x + i * 2 * THREADS_PER_BLOCK] = 1000;
            shared_mem[threadIdx.x + i * 2 * THREADS_PER_BLOCK + THREADS_PER_BLOCK] = 1000;
    ...
    for (int k = 1; k <= min(LOG_MAX_SHARED, 32 - __builtin_clz(total_size)); k++) {
...
int log_padded_size = (padded_size == 1) ? 0 : (32 - __builtin_clz(padded_size - 1));
...
cudaMemcpyAsync(arrSortedGpu, d_arr, size * sizeof(DTYPE), cudaMemcpyDeviceToHost);
cudaFree(d_arr);
/* ==== DO NOT MODIFY CODE BELOW THIS LINE ==== */
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
